...
//     // Testbench interface — fill input BRAM before asserting start_i
//     // Testbench drives these only when done_o is low and start_i not yet asserted
//     input  wire        tb_csb_i,
//     input  wire        tb_web_i,
//     input  wire [8:0]  tb_addr_i,
//     input  wire [31:0] tb_din_i,

//     // Conv output BRAM interface — write port to conv_output_bram
...
//     // Testbench owns input BRAM when idle, engine owns it when running
...
//     // ---------------------------------------------------------------
//     // Coordinate logic (combinational)
//     // signed arithmetic for padding
//     // ---------------------------------------------------------------
...
//     // Output BRAM: byte addr = (out_y * 20 + out_x) * 8 + out_c
...
//     // ---------------------------------------------------------------
//     // Registered pipeline signals (for BRAM read latency)
//     // ---------------------------------------------------------------
...
//     // Output word buffer — pack 4 bytes before writing to BRAM
...
//         end
//         else begin
//             // Default: deassert all BRAM enables
//             eng_in_csb     <= 1'b1;
//             filt_csb       <= 1'b1;
//             conv_out_csb_o <= 1'b1;
//             conv_out_web_o <= 1'b1;
...
//                 // ------------------------------------------------
//                 // Issue reads to input BRAM and filter ROM
//                 // ------------------------------------------------
//                 S_LOAD_IN: begin
//                     // Filter ROM: always read (even OOB input still
//                     // needs filter val fetched, we just won't use it)
//                     filt_csb         <= 1'b0;
...
//                 // ------------------------------------------------
//                 // Wait one cycle for registered BRAM output
//                 // ------------------------------------------------
//                 S_WAIT_BRAM: begin
//                     state <= S_MAC;
//                 end
...
//                 // ------------------------------------------------
//                 // MAC: accumulate filter * (input + 128)
//                 // ------------------------------------------------
//                 S_MAC: begin
//                     if (in_bounds_r)
//                         acc <= acc + $signed(filter_val)
//                                    * $signed(input_val);
...
//                 // ------------------------------------------------
//                 // Arithmetic right shift + round + offset + clamp
//                 // total shift = 31 + SHIFT[out_c]
//                 // ------------------------------------------------
//                 S_QUANT_SHIFT: begin
//                     // Using the same rounding as your C code:
//                     // prod += (prod >= 0) ? (1<<30) : -(1<<30)
//                     // prod >>= 31
//                     // then >> SHIFT[out_c] for the remaining shift
...
//                 // ------------------------------------------------
//                 // Pack byte into word buffer, write full words
//                 // ------------------------------------------------
//                 S_WRITE: begin
//                     // Place byte into correct lane of buffer
//                     out_word_buf[out_byte_lane*8 +: 8] <= quant_result[7:0];

//                     // Write to conv output BRAM when word is full
